"""以圖元群組鄰接關係及 BFS 產生「品名、尺寸、材質、數量」BOM。
...
TUBE_COLOR = "顏色_30"
TEE_SPECIAL_NAMES = {
    "union r.tee", "vcr r.tee", "union r tee", "vcr r tee",
    "union reducing tee", "vcr reducing tee",
}
GAUGE_NAMES = {"gauge", "guage"}
HOSE_NAMES = {"hose"}
REDUCER_NAMES = {"reducer"}
...
def build_entity_groups(df: pd.DataFrame, tolerance: float) -> list[EntityGroup]:
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="用圖元組鄰接矩陣與 BFS 產生管線 BOM")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.component_tolerance < 0 or args.connection_tolerance < 0:
        raise ValueError("容許距離不可小於 0")
    if not args.input.is_file():
        raise FileNotFoundError(f"找不到輸入檔案：{args.input}")

    output = args.output or args.input.with_name(f"{args.input.stem}_network_bom.csv")
